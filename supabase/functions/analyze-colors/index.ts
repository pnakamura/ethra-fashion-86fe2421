...
// Helper for API call with retry
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        const waitTime = Math.pow(2, attempt) * 1000;
        console.log(`[analyze-colors] Network error, retry ${attempt + 1}/${maxRetries} after ${waitTime}ms`);
        await new Promise(resolve => setTimeout(resolve, waitTime));
      }
    }
  }
...
4. Contraste geral entre pele, olhos e cabelo`;
    ...
    const response = await callAIWithRetry(LOVABLE_API_KEY, requestBody);
