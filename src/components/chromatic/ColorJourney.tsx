...
export function ColorJourney({ hasAnalysis, hasExplored = false, hasMakeup = false, hasLooks = false }: ColorJourneyProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-xl bg-card border border-border shadow-soft">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h4 className="text-sm font-medium">{t('journey.title')}</h4>
          <p className="text-xs text-muted-foreground">{t('journey.stepsCount', { completed: completedCount, total: steps.length })}</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-16 h-1.5 bg-secondary rounded-full overflow-hidden">
            <motion.div initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.5, delay: 0.2 }} className="h-full bg-gradient-to-r from-primary to-gold rounded-full" />
          </div>
