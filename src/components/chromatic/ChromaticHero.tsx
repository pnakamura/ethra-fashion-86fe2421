...
export function ChromaticHero({ 
  ...
  const getGradientColors = () => {
    if (displaySeason) {
      return displaySeason.colors.primary.slice(0, 4).map(c => c.hex);
    }
  ...
  if (!analysis && !isUsingTemporary) return null;
  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden rounded-2xl bg-card border shadow-soft ${
        isUsingTemporary ? 'border-amber-500/50 ring-2 ring-amber-500/20' : 'border-border/50'
      }`}
    >
      ...
      <div className="relative z-10 p-5">
          ...
          <div className="flex-1 min-w-0">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {isUsingTemporary && (
                  <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-600 dark:text-amber-400 text-xs font-medium">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mt-4 flex items-center gap-3">
          <div className="flex -space-x-1.5">
            {displaySeason?.colors.primary.slice(0, 8).map((color, i) => (
              <motion.div key={color.hex} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3 + i * 0.03 }} className="w-5 h-5 rounded-full border-2 border-card shadow-sm" style={{ backgroundColor: color.hex }} title={color.name} />
          ...
          <span className="text-xs text-muted-foreground">
            {displaySeason 
              ? t('hero.mainColors', { count: displaySeason.colors.primary.length })
        ...
        {!isUsingTemporary && wardrobeStats && wardrobeStats.total > 0 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mt-4 p-3 rounded-xl bg-secondary/50">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs text-muted-foreground">{t('hero.closetCompatibility')}</p>
              <span className="text-sm font-semibold text-primary">{compatibilityPercent}%</span>
            </div>
            
            <div className="h-2 bg-secondary rounded-full overflow-hidden flex">
              <motion.div initial={{ width: 0 }} animate={{ width: `${(wardrobeStats.ideal / wardrobeStats.total) * 100}%` }} transition={{ duration: 0.5, delay: 0.5 }} className="h-full bg-emerald-500" />
              <motion.div initial={{ width: 0 }} animate={{ width: `${(wardrobeStats.neutral / wardrobeStats.total) * 100}%` }} transition={{ duration: 0.5, delay: 0.6 }} className="h-full bg-amber-500" />
              <motion.div initial={{ width: 0 }} animate={{ width: `${(wardrobeStats.avoid / wardrobeStats.total) * 100}%` }} transition={{ duration: 0.5, delay: 0.7 }} className="h-full bg-rose-500" />
            </div>
            
            <div className="flex items-center justify-between mt-2 text-xs">
              <button onClick={() => navigate('/wardrobe?filter=ideal')} className="flex items-center gap-1 text-emerald-600 hover:underline">
                <Star className="w-3 h-3" />
                {wardrobeStats.ideal} {t('hero.ideal')}
              </button>
              <button onClick={() => navigate('/wardrobe?filter=neutral')} className="flex items-center gap-1 text-amber-600 hover:underline">
                <Shirt className="w-3 h-3" />
                {wardrobeStats.neutral} {t('hero.neutral')}
              </button>
              <button onClick={() => navigate('/wardrobe?filter=avoid')} className="flex items-center gap-1 text-rose-600 hover:underline">
                <AlertTriangle className="w-3 h-3" />
                {wardrobeStats.avoid} {t('hero.avoid')}
              </button>
            </div>
          </motion.div>
        )}
