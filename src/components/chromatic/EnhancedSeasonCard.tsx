...
export function EnhancedSeasonCard({ 
  ...
  if (variant === 'compact') {
    return (
      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={onClick} className={getCardClasses()}>
        <div className="p-3 flex items-center gap-3">
          <div className="flex -space-x-1.5">
            {season.colors.primary.slice(0, 4).map((color) => <div key={color.hex} className="w-6 h-6 rounded-full border-2 border-card shadow-sm" style={{ backgroundColor: color.hex }} />)}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium truncate">{season.name} {season.subtype}</p>
            <p className="text-xs text-muted-foreground truncate">{season.shortDescription}</p>
          </div>
  ...
  return (
    <motion.button whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }} onClick={onClick} className={getCardClasses()}>
      <div className="h-2 w-full" style={{ background: `linear-gradient(90deg, ${season.colors.primary.slice(0, 6).map(c => c.hex).join(', ')})` }} />
      <div className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div className="flex -space-x-2">
            {season.colors.primary.slice(0, 5).map((color, i) => <motion.div key={color.hex} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: i * 0.05 }} className="w-8 h-8 rounded-full border-2 border-card shadow-sm" style={{ backgroundColor: color.hex }} />)}
          </div>
          <span className="text-2xl">{season.seasonIcon}</span>
        </div>

        <div className="mb-2">
          <h3 className="font-display text-lg font-semibold text-foreground">{season.name} {season.subtype}</h3>
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            ...
            {isExperimenting && !isUserSeason && (
              <span className="inline-flex items-center gap-1 text-xs text-amber-600 dark:text-amber-400 font-medium px-2 py-0.5 rounded-full bg-amber-500/10">
                ...
                <span className="animate-pulse">{t('temporaryPreview.preview')}</span>
              </span>
            )}
        ...
        <p className="text-sm text-muted-foreground line-clamp-2 mb-3">{season.shortDescription}</p>

        <div className="flex flex-wrap gap-1 mb-3">
          {season.keywords.slice(0, 3).map((keyword) => (
            <span key={keyword} className="text-xs px-2 py-0.5 rounded-full bg-secondary text-secondary-foreground">{keyword}</span>
          ))}
        </div>

        <div className="flex items-center gap-2 text-xs text-muted-foreground pt-2 border-t border-border">
          <Users className="w-3 h-3" />
          <span className="line-clamp-1">{season.celebrities.slice(0, 2).join(', ')}</span>
        </div>
      </div>
    </motion.button>
  );
