...
import { Sparkles, Check, Minus, AlertTriangle } from 'lucide-react';
...
export function LookHarmonyBadge({
  items,
  size = 'md',
  showPercentage = false,
  harmonyType,
  className,
}: LookHarmonyBadgeProps) {
  const stats = {
  ...
  const idealPercentage = total > 0 ? Math.round((stats.ideal / total) * 100) : 0;
  const harmonyScore = total > 0 
      ...
      <Tooltip>
        ...
        <TooltipContent side="top" className="max-w-xs">
          <div className="space-y-2 text-xs">
            ...
            {harmonyType && (
              <p className="text-muted-foreground">Tipo: {harmonyType}</p>
            )}
          </div>
        </TooltipContent>
      </Tooltip>
