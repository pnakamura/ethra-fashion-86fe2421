...
export function TripCard({ trip, onClick }: TripCardProps) {
  ...
  const totalItems = trip.packing_list
    ? [...trip.packing_list.roupas, ...trip.packing_list.calcados, ...trip.packing_list.acessorios, ...trip.packing_list.chapeus].length
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
      <Card className="p-4 cursor-pointer border-0 shadow-soft hover:shadow-md transition-all group" onClick={onClick}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 text-sm text-muted-foreground">
              <span className="flex items-center gap-1">
                ...
                {format(new Date(trip.start_date), 'd MMM', { locale: dateFnsLocale })}
              </span>
              <span>•</span>
              <span>{tripDays} {t('tripList.days')}</span>
              <span>•</span>
              <span className="flex items-center gap-1">
                <Shirt className="w-3.5 h-3.5" />
                {totalItems} {t('tripList.items')}
              </span>
            </div>
          </div>
        ...
        <div className="mt-3 pt-3 border-t border-border/50">
          <Badge variant="outline" className="text-xs">
            {t(`tripTypes.${trip.trip_type}`, { defaultValue: trip.trip_type })}
          </Badge>
        </div>
      </Card>
    </motion.div>
