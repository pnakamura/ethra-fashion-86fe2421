...
export interface LocationOption {
  lat: number; lon: number; name: string; admin1?: string;
  country: string; country_code: string; flag: string; display_name: string;
}

interface LocationPickerProps {
  open: boolean; onOpenChange: (open: boolean) => void;
  locations: LocationOption[]; searchTerm: string;
  onSelectLocation: (location: LocationOption) => void; isLoading?: boolean;
}
...
export function LocationPicker({ open, onOpenChange, locations, searchTerm, onSelectLocation, isLoading = false }: LocationPickerProps) {
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t('locationPicker.which', { term: searchTerm })}
          </DialogTitle>
          ...
          <ScrollArea className="max-h-80">
            <div className="space-y-2">
              {locations.map((location, index) => (
                <motion.button key={`${location.lat}-${location.lon}`}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm truncate">{formatLocationLabel(location)}</p>
        ...
        <div className="flex justify-end pt-2">
          <Button variant="ghost" onClick={() => onOpenChange(false)} className="rounded-xl">{t('locationPicker.cancel')}</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
