...
interface LookSuggestion {
  look_name: string;
  items: string[];
  item_details?: Array<{ id: string; name: string; category: string; image_url: string }>;
  explanation: string;
  styling_tips?: string;
  score?: number;
  color_harmony?: { palette_name: string; colors_used: string[]; harmony_type: string };
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {suggestion.score && (
            <Badge variant="secondary" className="text-xs">
              <Sparkles className="w-3 h-3 mr-1" />{suggestion.score}%
            </Badge>
          )}
        </div>
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className="flex gap-0.5 ml-1">
              {suggestion.color_harmony.colors_used.slice(0, 4).map((color, i) => (
                <span key={i} className="w-3 h-3 rounded-full border border-white/20" style={{ backgroundColor: color }} />
              ))}
            </div>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-1.5">
          {suggestion.weather_consideration && (
            <Badge variant="outline" className="text-[10px] h-5">
              <ThermometerSun className="w-2.5 h-2.5 mr-1" />{suggestion.weather_consideration}
            </Badge>
          )}
          {suggestion.dress_code_match && (
            <Badge variant="outline" className="text-[10px] h-5">
              <CheckCircle className="w-2.5 h-2.5 mr-1" />{suggestion.dress_code_match}
            </Badge>
          )}
        </div>
      </div>
            ...
            <div className="flex gap-2 pb-1">
              {suggestion.item_details.map((item, index) => (
                <div key={index} className="flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-muted">
                  <img src={item.image_url} alt={item.name || item.category} className="w-full h-full object-cover" />
                </div>
              ))}
            </div>
      ...
      {suggestion.styling_tips && (
        <div className="px-4 py-3 border-t border-border/30 bg-amber-500/5">
          <div className="flex items-start gap-2">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-400">{suggestion.styling_tips}</p>
          </div>
        </div>
      )}
      ...
      {suggestion.improvements && suggestion.improvements.length > 0 && (
        <div className="px-4 py-3 border-t border-border/30 bg-blue-500/5">
          <div className="flex items-start gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-blue-700 dark:text-blue-400">{t('lookSuggestion.canImprove')}</span>
          </div>
          <ul className="space-y-1 pl-5">
            {suggestion.improvements.map((improvement, i) => (
              <li key={i} className="text-xs text-blue-600/80 dark:text-blue-400/80 list-disc">{improvement}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      {suggestion.event_type_tips && (
        <div className="px-4 py-3 border-t border-border/30 bg-purple-500/5">
          <div className="flex items-start gap-2">
            ...
            <p className="text-xs text-purple-700 dark:text-purple-400">{suggestion.event_type_tips}</p>
          </div>
        </div>
      )}

      <div className="p-3 border-t border-border/30 flex gap-2">
        <Button variant="outline" size="sm" className="flex-1 h-8 text-xs rounded-lg" onClick={handleViewOnCanvas}>
          <Eye className="w-3 h-3 mr-1" />{t('lookSuggestion.assembleLook')}
        </Button>
        <Button size="sm" className="flex-1 h-8 text-xs rounded-lg gradient-primary" onClick={handleTryOn}>
          <Sparkles className="w-3 h-3 mr-1" />{t('lookSuggestion.tryOn')}
        </Button>
      </div>
    </motion.div>
  );
